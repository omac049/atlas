...
> changed.** Every one of the first 18,650 gap observations priced a
> **Polymarket Global** leg: offshore, no published order book, and declared
> non-executable by its own adapter. The radar now also watches **Polymarket
> US** — the venue a US account can reach, which publishes a two-sided book so
> both legs can be sized for real. Against that venue the picture is different:
> **36 twin-shaped pairs, 2 executable, both GDP Q3 at under a cent**; a
> hand-run snapshot across FOMC Sep/Oct, CPI YoY, and unemployment found **27
> pairs and 0 executable**, with baskets costing $1.00–$1.02. The two US venues
> are arbitrage-linked and agree on price. Every tradeable executable gap on
> 2026-08-20 was **below the floors** — a 7.8c GDP gap was backed by **0.06
> contracts** of Polymarket depth.
>
> The study's go/no-go now charges for capital lock-up (the phase-2 metric,
> pulled forward from day 31) and **flips GO → NO-GO on unchanged data**:
> **3.38% annualized** on locked capital overall, **1.66%** on the tradeable
> venue, median basket **$459**. On that evidence the owner **dropped the
> execution track and pivoted to contract intelligence**. The staged path in
> "Beyond paper-only" below is now **dormant, not pending** — paper-only stands
> indefinitely. Details: `docs/NINETY_DAY_STUDY.md` (Amendments, 2026-08-20).

Last verified: **2026-08-20** (full suite + `ruff check` + live API/dashboard read).
...
- Test suite: `616 passed, 1 warning` at the latest full run; `ruff check .` reports zero issues.
- **The contract-intelligence deliverable exists (2026-08-24): the weekly Contract Divergence Report** (`atlas intel report`, `atlas/intel.py`, Mondays 07:15 via `com.atlas.intel` into `data/intel/` as markdown + JSON). It assembles what Atlas knows about contracts — approved equivalents awaiting settlement, pairs blocked only by venue rules text, a per-family rules-completeness scorecard (Kalshi vs Polymarket), settlement-timing asymmetries with capital lock-up horizons, and latest cross-venue price disagreements flagged by tradeability and tick/size floors. Read-only over persisted evidence, regenerable from the database, decides nothing; every section carries its own caveats. This is the first artifact of the 2026-08-20 pivot and the thing phase-3 user interviews show to prospective users.
- **The Settlement Clarity Score (2026-08-24): a deterministic A–F grade of any single contract's fine print** (`atlas clarity grade`, `atlas clarity scan`, `atlas/clarity.py`). It needs no twin pair, no order book, and no model: it reads the venue's own published text through the existing deterministic assessors and subtracts fixed, published points for each payout branch the text leaves undetermined — missing authoritative source, no cancellation terms, no revision rule, no terminal missing-data branch — with plain English and a "what the venue would have to publish" line for every deduction. A discretionary fair-price clause caps the grade at D no matter how complete the rest is. First live sweep (2026-08-24, 2,000 open contracts per venue): **Kalshi mean 20.3 (0 A, 0 B, 0 C, 727 D, 1,273 F), Polymarket US mean 38.4 (17 C, 1,521 D, 462 F)** — and the two means are **not** cross-venue comparable, because Kalshi publishes its settlement sources on the event record this sweep does not read; the artifact says so alongside the numbers. Paper-only, observability-only: the score is a read-only consumer of the frozen settlement/verification pipeline, an import-graph test pins that nothing in that pipeline can ever import it back, and a grade decides nothing. Dated scans land in `data/clarity/`; the divergence report renders them when a scan exists and omits the section when none does.
...
- An approval-frontier watch (`atlas pairs frontier`, also `approval_frontier` in `/api/overview`) ranks blocked pairs by how far they are from approval and flags pairs whose published rules text has changed, so waiting for venue-text alignment is no longer passive. It separates blockers a venue could clear by publishing more text from structural divergences that no amount of waiting fixes, and it reports its own blind spots. The monitor now records a rules baseline for both legs of every blocked candidate (`capture_frontier_rules_evidence`), including Polymarket Global legs the validation universe never sees. Latest scan: 8 blocked, 6 blocked on venue text alone, 0 rules changes in 14 days, **0 pairs with an unmonitored leg** — every blocked pair has a baseline, so the next venue text revision registers as a rules-version change.
- A paper-only gap radar (`atlas gaps scan --live`) now measures live cross-venue price gaps on twin-shaped candidate pairs at executable top-of-book prices, and the dashboard's "$2k paper meter" answers the original bankroll question with recorded assumptions instead of hope. The meter stakes 5% of the current bankroll per opportunity, capped by the **thinner** leg's displayed depth and **skipping** any opportunity whose depth no venue published (previously those ran uncapped and supplied 35% of the recorded profit), so it measures compounding, the radar watches every canonically normalized family on both venues (21 twin-shaped pairs across 11 subjects at the 2026-08-20 scan, up from 17 when chamber control was added), and the monitor bursts the read-only radar to a 30-second cadence inside scheduled release windows (`atlas/release_calendar.py`).
- **The 90-day opportunity study is running (day 2 of 90; decision date 2026-11-17).** Charter and metric definitions: `docs/NINETY_DAY_STUDY.md`; metrics in `atlas/study.py`; weekly artifact written by `atlas gaps study` (Mondays 07:00 via `com.atlas.study`) into `data/study/`. It asks whether verified cross-venue opportunities occur often enough, survive long enough, and carry enough size to justify ever building execution — or whether Atlas's durable value is the contract intelligence itself. The deterministic verifier and normalizers are **frozen for measurement** for the duration; changes need owner sign-off plus an amendment note. Latest report (2026-08-20, after the Polymarket US amendment): 26 distinct opportunities from **11 distinct pairs**, 73.3 verified-precursor opportunities per 30 days against a frequency threshold of 10 — but `meets_go_threshold.go` is **false**, because return on locked capital reads 3.38% annualized against a 15% hurdle and median basket notional $459 against a $500 floor. Tradeable (Polymarket US) subset: 2 opportunities, 1.66% annualized. Median survival 144.8 minutes.
- Settlement-timing asymmetry is measured, not guessed (`atlas/settlement_timing.py`): venue rules text is read for early-determination clauses (Kalshi's chamber-control contracts may settle on "a consensus of media calls" while the Polymarket twin waits on official sources), and gap-radar observations carry the tag plus the capital lock-up horizon — the **later** leg's anchor, because a paired position frees capital only when both legs settle. Observability only: the annotation gates nothing and can never promote a pair. The study's gap-vs-horizon curve separates rows recorded before the annotation shipped from rows whose venues published no anchor, and reports `asymmetry_blind_spot` when no watched pair can carry the tag — a null asymmetric median is a missing test, never a finding. **The radar learned categorical twins on 2026-08-20** (`_twin_shape` in `atlas/gap_radar.py`): chamber-control contracts publish no threshold, so the threshold-only matcher could never pair them and the asymmetry split had no eligible population at all. Categorical twins now pair on identical subject/action/scope plus a non-null, equal affirmative outcome — never opposing parties (ties exist, so a complement would be inference), and never a joint 'Balance of Power' contract against a single-chamber bet. The two chamber families are quarantined from the go/no-go rate because they entered scope mid-study; they are measured in full under `post_start_scope`.
- Trusted settlement labels: `81` — 10 `APPROVED_EQUIVALENT` + 71 evidence-backed `REJECTED`. **Both the label-mix and the 50-label volume requirements are satisfied**; the learning loop reports `READY` with no blockers, so the next move is a first training/evaluation experiment against the frozen holdout rather than more accumulation.
- Latest learning readiness check: `training_ready=True`, `labels=81`, `observations=388`.
...
- **The live label loop works for the first time (2026-08-20).** The scan compared Kalshi against Polymarket US only, while every approved pair on record is Kalshi x Polymarket Global — so `discovery_scans.approved` read 0 across 21 consecutive scans while 4 approved pairs sat in the queue, and 100% of trusted labels came from `backfill.py`. After widening the pairing universe: comparisons 760 -> 1,182, approved **0 -> 12**, validation cases 81 -> **112**, including the first `source_kind=APPROVED` cases the live path has ever created. Reconciliation now routes each leg to its own venue adapter, so a Global leg no longer 404s against the US gateway and retries forever.
...
The first-trusted-pair milestone was completed on 2026-08-13 and the balanced trusted-dataset milestone on 2026-08-14. The active milestone is now the **90-day opportunity study** (adopted 2026-08-19, decides 2026-11-17), which governs what changes are allowed while it runs: the verifier and normalizers are frozen for measurement, and frozen-holdout evaluation plus settlement-frontier improvement continue underneath it. The next dated commitment is **phase 2 by day 31 (2026-09-18)** — latency-adjusted shadow replay at 250ms/500ms/1s/2s against the next recorded quotes, reporting return on locked capital until settlement rather than profit per basket. It needs burst sampling around detected gaps, since a 5-minute sweep cannot resolve sub-second decay; that is an instrumentation addition, not a rule change. A trusted pair requires that it:
