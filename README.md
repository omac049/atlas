...
- Trusted settlement labels: `34` — 8 `APPROVED_EQUIVALENT` + 26 evidence-backed `REJECTED`. **The label-mix requirement is satisfied**; the learning loop waits on volume only (need 16 more). The settled cross-venue archive is fully harvested (2026-08-13: progressively deeper bounded passes over 14 Fed meetings and 21 CPI months converged to zero new labels); the remainder accrues from the calendar (~8 FOMC meetings/year, monthly CPI releases), watched automatically.
