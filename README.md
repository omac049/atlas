...
- Test suite: `307 passed, 1 warning` at the latest full run; `ruff check .` reports zero issues.
...
- Trusted settlement labels: `38` — 8 `APPROVED_EQUIVALENT` + 30 evidence-backed `REJECTED` across six canonical families (FOMC decision, fed-funds level, CPI ×4, crypto, unemployment U3, ISM PMI ×2). **The label-mix requirement is satisfied**; the learning loop waits on volume only (need 12 more), which September's release calendar (ISM 1st/3rd, jobs report 4th, CPI 11th, FOMC 16th) supplies automatically via the running monitor.
- Latest learning readiness check: `training_ready=False`, `labels=34`, `observations=331`.
