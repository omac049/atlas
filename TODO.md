...
- [x] **Quality pass (2026-09-24, same day).** First live run: the top-100 lexical shortlist came from 4 of 392 Kalshi events, only 5/25 proposals were true twins (P≈0.65, dragged down by a "judged the same way" criterion vs. a low same-source Noul), and 20/25 sat at P≈0.01. Now: `jev_shortlist` caps 5 per event / 2 per market and drops pairs whose parsed lines differ under the *same* operator (numbers stay in code, per Jev's docs); the relation question ignores the resolution source; state is compact (title, outcome, close date, 1,500 chars of rules); P(same) < 0.2 is not proposed. Live on the same catalogs: **25/25 exact twins at P ≥ 0.98 across 13 events**; 100 Jev calls in ~7s.
- [x] **Agent loop:** `verify_candidates` ran until `step_limit_reached` whenever nothing was approved (4× on the live run). It now verifies once and stops.
- [x] **Measured against trusted labels** — `atlas agent evaluate-proposer` (research only; 95 settlement-verified labels, 10 approved / 85 rejected): ROC AUC **title overlap 0.665 → Jev 0.919 → Jev + number check 0.934**. At the 0.2 floor Jev keeps 4/10 approved and 1/85 rejected (title overlap: 10 and 49).
- [x] Caught by the measurement: the first number check (any differing line) dropped the six Fed grid twins ("Hike >25bps" ≡ "50+ bps", `docs/decisions/2026-08-12-fed-rounding-preimage-equality.md`) and cut AUC to 0.672. It now requires the same operator on both legs. Pinned by test.
- Known limit: Jev scores those six grid twins ≈0.05 — equivalence on a rounding grid is arithmetic, which Jev reads literally. The agent's verify step still runs the deterministic full-catalog scan, which is what finds them.
- Known limit: the live twins Jev finds are NFL player props; the frozen verifier has no normalizer for them (8 mismatch codes each), so none can be approved. Teaching it is a frozen-path change needing sign-off.
...
- [x] **`docs/decisions/2026-08-31-fine-print-proof-charter.md` drafted.** The
  theory ("low clarity grades predict settlement trouble") now has three tests
  with thresholds fixed BEFORE data: Test A (≥15 public disputes vs matched
  controls; median gap ≥10 points AND trouble-finding rate ≥2x), Test B
  (settlement delay ≥2x for fallback-less/discretionary markets, ≥200 settled
  markets), Test C (seven pre-registered September predictions, C1–C7, each
  with a named trigger; grades quoted from live v1.1 output on 2026-08-31).
  Circularity trap documented: never test against Atlas's own labels.
- [x] Decision rule fixed: PROVEN needs A plus (B or a C hit); an unpredicted
  dispute on a watched market DISPROVES; inconclusive is not permission.

- [ ] **OWNER: merge the charter PR before 2026-09-04** — merging constitutes
  sign-off and arms Test C. After Sep 4 the jobs predictions can no longer be
  pre-registered.
- [ ] Assemble the Test A corpus (≥15 citable disputes + rule-selected
  controls) — next build.
- [ ] Test B bounded settled-market fetch (stated vs actual settlement times,
  ≥200 rows) — after the corpus.
- [ ] Score C1–C7 after each release; write results into the charter with
  evidence, including nulls.
...
## 2026-08-24 (b) — the 05:30 problem and the LAN bind

- [x] **`com.atlas.awake`** (`deploy/atlas_awake.py` + plist, installed and
  loaded): holds a `caffeinate -si` assertion from 10h before each scheduled
  release through the end of its burst window, driven by `keep_awake_window` /
  `next_keep_awake_start` in `atlas/release_calendar.py` — one calendar, so the
  burst scans and the hold can never disagree. Verified live: agent idle now,
  next hold opens 2026-08-31 21:00 local for the Sep 1 ISM print; the Sep 4
  jobs hold opens Sep 3 19:30 local. Limits stated in the module docstring and
  deploy README: cannot wake a sleeping Mac, cannot survive a closed lid — lid
  open + AC required; the optional root `pmset repeat wakeorpoweron` composes
  with it for guaranteed wakes.
- [x] **API `0.0.0.0` bind committed as an owner decision** (2026-08-24). The
  installed plist already carried it, so this records reality. Exposure: a
  read-only, unauthenticated research API on the local network — no credentials,
  no order path. Rationale + revert instructions in deploy/README.md.

- [ ] The release calendar ends at the 2026-12-09 FOMC; the awake agent logs
  "no future releases" past it. Add Q1-2027 entries before then.
...
- [ ] Fifth charter (market making, paper-only replay): signed 2026-09-08 (#32); instrument frozen 2026-09-15 at `383d609` (#65); **Arm B done 2026-09-15: FAIL** (352 games, net −$4,492.52, 25.6% of markets positive, −1.98¢/contract; artifact `docs/proof/making-result-armB.json.gz`; the §11.3 hold was broken by accident and is recorded in the charter); **Arm A 2026-09-17: NOT RUN — its recorded books are invalid** (charter note of that date; INCONCLUSIVE under §5, no fill/fee/P&L computed); the single widened run on 2026-10-29 reads `data/making/books.sqlite3` *if* the owner signs the amendment; archive the October tapes before Kalshi's ~6-week print retention reaches the window's start (first pass 2026-10-13, final fetch right after the 2026-10-28 close); after the run: write the result into the charter and `docs/FINDINGS.md`, remove `com.atlas.books` and the Fed-decision prune exemption (#64)
...
- [ ] Fee Verified affiliate programs: **Shopify live 2026-09-09**, **Intuit/QuickBooks live 2026-09-11**; Square/Impact applied 2026-09-11, awaiting approval; Wise/Partnerize blocked on the account website reading verifiedfeeds.com (owner: fix it and verify the Partnerize email); Printful, Printify and Vendoo have no page, so nothing to place. Placement rule and per-partner terms: `feeverified/site.py` PARTNERS
- [ ] Fee Verified weekly scoreboard (Mondays, alongside samebetornot.com) in the charter; verdict at week six
- [x] Loop 1, Search Console pulled nightly (`docs/GSC.md`, #59): built, merged and set up 2026-09-11; first automated pull logged in both scoreboards; `com.atlas.gsc` runs daily at 06:15
- [ ] verifiedfees.com indexing: only the home page is in Google's index (7 of 7 sampled inner pages unknown, 2026-09-11). Owner: request indexing in Search Console for the head pages; re-check the sample in a few days
- [ ] Fee Verified build window closes 2026-09-22. Built beyond the round-two list: QuickBooks Payments (18), Grailed (19), TikTok Shop, Upwork and Patreon (20 to 22). Deferred candidates with reasons in the charter: Facebook Marketplace (fee not machine-readable), TCGplayer (own help center holds the head term; one published example inconsistent), Clover (rates vary by reseller)
- [x] Fee Verified Safe Browsing false positive (2026-09-08): "Deceptive pages" site-wide; false-positive report + Search Console review 2026-09-08 (owner); **cleared 2026-09-09** (public status: no unsafe content found)
